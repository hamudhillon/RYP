...
app.get("/welcome",(req,res)=>{
	...
	MongoClient.connect(url,function(err,db){
		if (err) throw err;
		var dbs=db.db('ryp');
		dbs.collection("users").find({"Email":req.query.uname}).toArray(function(err,ress){
		if (err) throw err
		else{
			// console.log(ress.toString());
			// console.log(JSON.stringify(ress));
				...
				res.render(__dirname+"/"+"welcome",{id:idd});
				// return idd;
				// res.write(idd);
			// console.log(j[0]['_id']);
			// ejs.render('welcome',{id:idd};
...
app.post('/tdata',(req,res)=>{
	...
	MongoClient.connect(url,(err,db)=>{
		...
		dbs.collection("tdata").find({'department':dep,'course':course}).toArray((err,ress)=>{
			if (err) throw err
		else{
				var data=JSON.stringify(ress);
				var j=JSON.parse(data);
